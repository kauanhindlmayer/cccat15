...
<template>
  <span class="step" @click="signupForm.setData()">
  ...
  <div v-if="signupForm.step === 1">
    <input
      class="input-is-passenger"
      type="checkbox"
      v-model="signupForm.isPassenger"
    />
    Passenger
  </div>
  <br />
  ...
  <button
    v-if="signupForm.showPrevious()"
    class="button-previous"
    @click="signupForm.previous()"
  >
    Anterior
  </button>
  <button
    v-if="signupForm.showNext()"
    class="button-next"
    @click="signupForm.next()"
  >
    Próximo
  </button>
  <button
    v-if="signupForm.showSubmit()"
    class="button-submit"
